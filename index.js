...
const leia = makePersonObject(5, 'Leia', 'leia@leia.com');
/**
 * ### Challenge `getName`
 * 
 * @instructions
...
function getCarInfoByIndex(inventory, index) {
  ...
  const make = info.car_make;
  const model = info.car_model;
...
function getGermanCars(inventory) {
  var german = [];
  for (let i = 0; i < inventory.length; i++) {
    if (inventory[i].car_make === 'Audi' || inventory[i].car_make === 'Volkswagen' || inventory[i].car_make === 'BMW' || inventory[i].car_make === 'Mercedes-Benz') {
